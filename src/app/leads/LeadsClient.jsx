...
export default function LeadsClient({
  initial, statuses, sources, canImport,
    ...
    <div className="p-8">
          ...
          {tabs.map((t) => {
            ...
            return (
              ...
              >
                {t.label}
                {t.count != null ? (
                  <span className={"ml-1.5 text-xs " + (active ? "text-white/80" : "text-[#9AA6A4]")}>
                    {t.count}
                  </span>
                ) : null}
              </button>
      ...
      {canImport && bulkOpen ? (
        <div className="mt-4 rounded-lg border border-[#E4E1DA] bg-white p-5">
          <h2 className="text-sm font-semibold text-[#14201F]">Import leads from CSV</h2>
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            <div>
              <label className="mb-1.5 block text-xs font-medium text-[#14201F]">Source</label>
              <select className={fieldCls} value={sourceId} onChange={(e) => setSourceId(e.target.value)}>
                <option value="">Select source...</option>
                {sources.map((s) => (<option key={s.id} value={s.id}>{s.name}</option>))}
              </select>
            </div>
            <div>
              <label className="mb-1.5 block text-xs font-medium text-[#14201F]">Campaign name</label>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3">
        <input value={searchInput} onChange={(e) => setSearchInput(e.target.value)}
          placeholder="Search #id, name or phone..." className={fieldCls + " max-w-xs"} />
        <select className={fieldCls + " max-w-[200px]"} value={statusId} onChange={(e) => go({ status: e.target.value, page: 1 })}>
      ...
      {/* List */}
      {items.length === 0 ? (
        <div className="mt-16 text-center"><p className="text-sm text-[#6C7A78]">No leads match.</p></div>
      ) : (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="truncate text-xs text-[#6C7A78]">{l.phone}</div>
                  </div>
                </div>

                <div className="min-w-0">
                  <div className="truncate text-sm text-[#14201F]">{req || "-"}</div>
                  {budget ? <div className="truncate text-xs text-[#6C7A78]">{budget}</div> : null}
                </div>

                <div className="truncate text-sm text-[#6C7A78]">{l.source_name || "-"}</div>
                ...
                <div className="min-w-0">
                  ...
                  {follow ? (
                    <div className={"truncate text-xs " + (follow === "Today" ? "font-medium text-[#C58A12]" : "text-[#9AA6A4]")}>
                      {follow}
                    </div>
                  ) : null}
                </div>
          ...
          {pageNums.map((n, i) =>
            n === "..." ? (<span key={"e" + i} className="px-2 text-sm text-[#9AA6A4]">...</span>) : (
              <button key={n} onClick={() => go({ page: n })}
                className={"rounded-md px-3 py-1.5 text-sm transition " + (n === page ? "bg-[#0F1C1E] text-white" : "border border-[#E4E1DA] text-[#6C7A78] hover:border-[#1F7A6B]")}>
                {n}
              </button>
            ),
          )}
